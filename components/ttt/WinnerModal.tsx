...
export const WinnerModal = ({winner ,handleQuitGame , handleNewGame} :GameProps) => {
  return (
    <div className="bg-gray-900/90 z-10 min-h-screen w-full absolute top-0 left-0">
      <div className="w-[500px] h-[250px] rounded-xl bg-[#3f4045] space-y-10 px-6 py-4 mx-auto mt-52 flex items-center justify-center flex-col">
       <h2 className="flex flex-col items-center justify-center space-y-6 text-2xl md:text-4xl font-bold">
         {winner === "X" ? <XIcon /> : <OIcon />}
         <p className={winner === "X" ? "text-[#f6ae2d]" : "text-[#5da9e9]"}>Win the Round</p>
       </h2>
      ...
      <div className="flex items-center justify-center space-x-16">
        ...
        <button onClick={handleNewGame} className="button px-4 rounded-md py-1 bg-[#f6ae2d] hover:bg-[#5da9e9] hover:ring-4 hover:ring-cyan-300">Next Round</button>
      </div>
      </div>
    </div>
  )
